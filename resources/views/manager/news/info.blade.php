...
<div class="box box-info" style="padding:15px;">
    <div class="box-body">
        {{Form::model($news,['class'=>'form-horizontal form-bordered','id'=>'postForm','name'=>'postForm' ])}}
        {{Form::hidden('id',old('id'))}}
        <div class="row mt1">
            <div class="form-group">
                {{Form::label('title','新闻标题：',['class'=>''])}}
                {{Form::text('title',old('title'),['class'=>'form-control','style'=>'width: 360px;'])}}
            </div>
        </div>
		<div class="row mt1">
			<div class="form-group">
				{{Form::label('outer_link','外部链接：',['class'=>''])}}
				{{Form::text('outer_link',old('outer_link'),['class'=>'form-control','style'=>'width: 360px;'])}}
			</div>
		</div>
	    <div class="row mt1">
		    <div class="form-group">
			    {{Form::label('sorts','排序序号：',['class'=>''])}}
			    {{Form::text('sorts',old('sorts'),['class'=>'form-control','style'=>'width: 360px;'])}}
		    </div>
	    </div>
        <div class="row">
            <div class="form-group">
                {{Form::label('pic','电脑图片：',['class'=>'col-sm-1','style'=>'width:75px;padding:0px;'])}}
                <div class="col-sm-8" style="padding:0;">
                    <input id="uploadPic" type="file" name="image" class="file-loading" />
                </div>
                {{Form::hidden('pic',old('pic'))}}
            </div>
        </div>
        <div class="row mt1">
            <div class="form-group">
                {{Form::label('is_show','是否发布：',['class'=>''])}}
                {{Form::radio('is_show',0,old('is_show')==0,['class'=>'minimal'])}}不发布&nbsp;&nbsp;
                {{Form::radio('is_show',1,old('is_show')==1,['class'=>'minimal'])}}发布
                {{Form::radio('is_show',2,old('is_show')==2,['class'=>'minimal'])}}已结束
            </div>
        </div>
        <div class="row mt1">
            <div class="form-group">
                {{Form::label('begin_time','开始时间：',['class'=>'col-sm-1','style'=>'padding:0;width:75px'])}}
                <div class="col-sm-5" style="padding:0!important;width: 280px;">
                    {{Form::text('begin_time',old('begin_time'),['class'=>'form-control query_time', 'style'=>'width: 276px;','readonly'=>"readonly"])}}
                </div>
                {{Form::label('end_time','结束时间：',['class'=>'col-sm-1','style'=>'width:100px;'])}}
                <div class="col-sm-5" style="padding:0!important;width: 280px;">
                    {{Form::text('end_time',old('end_time'),['class'=>'form-control query_time','style'=>'width: 276px;','readonly'=>"readonly"])}}
                </div>
            </div>
        </div>
        {{Form::hidden('html',empty($news->content)?'':$news->content,['id'=>'html'])}}
        <div class="row mt2">
...
<script>
	...
	$("#uploadPic").fileinput({
		...
		initialPreviewConfig:JSON.parse('{!!json_encode($picConfig)!!}'),
