        ...
        <table id="tbl-activities" class="table table-hover">
            ...
            @foreach($commonReports as $key=>$commonReport)
                <tr>
                    ...
                    {{--<td>{{config('enums.room_code')[$commonReport->room_code]}}</td>--}}
                    <td class="t-c">{{config('enums.game_type_code')[$commonReport->game_type_code]??'-'}}</td>
                    <td class="hidden-xs">{{$commonReport->bet_count}}</td>
                    <td class="hidden-xs t-r">{{$commonReport->money_total}}</td>
                    <td class="t-r">{{$commonReport->money_use}}</td>
                    <td class="@if($commonReport->win_lose<0) text-red @endif t-r">
                        {{$commonReport->win_lose}}
                    </td>
                    <td class="hidden-sm hidden-xs t-c">{{$commonReport->calc_date}}</td>
                    <td class="t-c">
                        <a href="javascript:" class="text-blue"
							onclick="myaddTabs('userGamePlayList{{++$key+($page_count*($current_page-1))}}','投注明细','{{route('gameData')}}?player_name={{$commonReport->login_name}}&room_code={{$commonReport->room_code}}&startDate={{$commonReport->calc_date}} 00:00:00&endDate={{$commonReport->calc_date}} 23:59:59');">
                            明细
                        </a>
                    </td>
                </tr>
