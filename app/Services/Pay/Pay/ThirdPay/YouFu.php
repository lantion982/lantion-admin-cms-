...
class YouFu extends BasePay{
    public $flag;
    public $needActivity          = false;
    public $payment_platform_code = 'YouFu';
    ...
    public function doPayIn(){
        ...
        ksort($data);
        $data['sign'] = strtoupper(md5(urldecode(http_build_query($data) . '&key=' . $info_acct['md5key'])));
        $data['signType'] = 'MD5';
    ...
    private function _getPayType(){
        ...
        switch($this->payClient->paymentMethodCode){
            case 'AliPay':
                $pay_type = 'pay.alipay.native';
                break;
            case 'WeiXin':
                $pay_type = 'pay.weixin.native';
                break;
            case 'QQPay':
                $pay_type = 'pay.qq.native';
                break;
        }
        return $pay_type;
    }
